...
<div class=" input_tag2 col-md-12">
    @foreach($product->other_color()->get() as $other)
    <div class="row">
        <div class="col-md-5">
            {!! Form::label('color_key',  trans('admin.product_colors_S')) !!}
            {{ Form::select('color_key[]', App\Models\Color::pluck('name_ar','id'), $other->color_key, array('class'=>'form-control', 'placeholder'=>'Please select ...')) }}
        </div>
        <div class="col-md-5">
            {!! Form::label('color_show',  trans('admin.product_colors_S')) !!}
            {!! Form::text('color_show[]',$other->color_show, ['class' => 'form-control']) !!}
        </div>
        <a href="#" class="remove_input2 btn btn-danger"><i class="fa fa-trash"></i></a>
    </div>
        @endforeach
