            ...
            <div class="row">
                <div class="col-lg-6">
                    <div class="product-order">
                        @forelse (App\Models\OrderProduct::all()->where('order_id',$order->id) as $item)
                            <div class="row product-order-detail">
                                <div class="col-3"><img src="{{imageDo($item->product->main_image)}}" alt=""
                                                        class="img-fluid blur-up lazyload"></div>
                                <div class="col-3 order_detail">
                                    <div>
                                        <h4>{{trans('user.product_name')}}</h4>
                                        <h5>{{$item->product->title}}</h5>
                                    </div>
                                </div>
                                <div class="col-3 order_detail">
                                    <div>
                                        <h4>{{trans('user.colors')}}</h4>
                                        <h5>{{$item->color}}</h5>
                                ...
                                <div class="col-3 order_detail">
                                    <div>
                                        <h4>{{trans('user.product_price')}}</h4>
                        ...
                        <div class="total-sec">
                            <ul>
                                <li>{{trans('user.subtotal')}} <span>{{presentPrice($order->billing_subtotal)}}</span></li>
                                <li>{{trans('user.delivery')}} <span>{{presentPrice($order->billing_delivery)}}</span></li>
{{--                                <li>tax(GST) <span>{{presentPrice(0)}}</span></li>--}}
                            </ul>
                        </div>
                        <div class="final-total">
                            <h3>{{trans('user.total')}} <span>{{presentPrice($order->billing_total)}}</span></h3>
                        </div>
                    </div>
                </div>
                <div class="col-lg-6">
                        ...
                        <div class="row">
                            <div class="col-sm-6">
                                ...
                                <ul class="order-detail">
                                    <li>{{trans('user.order_number')}}: {{$order->id}}</li>
                                    <li>{{trans('user.order_date')}}:{{$order->created_at}}</li>
                                    <li>{{trans('user.total')}}: {{presentPrice($order->billing_total)}}</li>
                                </ul>
                            </div>
                            <div class="col-sm-6">
                                ...
                                <ul class="order-detail">
                                    <li>country:{{ $order->billing_country }}</li>
                                    <li>city:{{ $order->billing_city }}</li>
                                    <li>buliding:{{ $order->billing_buliding }}</li>
                                    <li>road:{{ $order->billing_road }}</li>
                                    <li>block:{{ $order->billing_block }}</li>
                                    <li>speical_direcstions:{{ $order->billing_speical_direcstions }}</li>
                                ...
                                <ul class="order-detail">
                                    ...
                                    <li>address2:{{ $order->billing_address2 }}</li>
                                    <li>province:{{ $order->billing_province }}</li>
                                    <li>postalcode:{{ $order->billing_postalcode }}</li>
                                    <li>phone:{{ $order->billing_phone }}</li>
                                </ul>
