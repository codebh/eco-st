...
$level= delivery()->user()->level;
    ...
    elseif ($level =='2') {
        $orders =Order::where('billing_country','bahrain')->where('delivery_type','2')->get();
        // return view('delivery.welcome',compact('orders'));
    }
    elseif ($level =='3') {

        $orders =Order::where('billing_country','bahrain')->where('delivery_type','1')->get();

    }
    elseif ($level =='4') {
        $orders =Order::where('delivery_type','1')
        ->where(function ($q) {
